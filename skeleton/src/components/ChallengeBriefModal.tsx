...
import type { Challenge, Difficulty, AttemptInfo } from '../types'

// ─── Colour maps (mirrors CompanyPage) ───────────────────────────────────────
...
export default function ChallengeBriefModal({
  challenge,
  onClose,
  onStart,
  isStarting,
      ...
      <div className="flex items-start justify-between gap-4 px-6 pt-6 pb-4 border-b border-[--color-border-hover] shrink-0">
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className={`text-[10px] px-2 py-px rounded-full font-medium ${DIFF_BADGE[challenge.difficulty]}`}>
              {challenge.difficulty}
            </span>
            <span className="text-[10px] font-mono text-[--color-text-muted] tabular">
              {challenge.time_budget_minutes} min
            </span>
